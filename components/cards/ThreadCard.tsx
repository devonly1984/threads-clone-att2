...
isComment}:Props) => {
  return (
    <article className={`${isComment ? 'px-0 xs:px-7' : "bg-dark-2 p-7"}flex w-full flex-col rounded-xl bg-dark-2 p-7`}>
            ...
            <div className="flex w-full flex-1 flex-row gap-4">
                ...
                <div className={`${isComment && 'mb-10'} mt-5 flex flex-col gap-3`}>
                    <div className="flex gap-3.5">
                        ...
                        <Link href={`/thread/${id}`}>
                            <Image src="/assets/reply.svg" alt="replay" width={24} height={24} className="object-contain cursor-pointer"/>
                        </Link>
                    ...
                    {isComment && comments.length >0 && (
                        <Link href={`/thread/${id}`}>
                            <p className="mt-1 text-subtle-medium text-gray-1">{comments.length}</p>Replies
                        </Link>
                    )}
                </div>
                </div>
